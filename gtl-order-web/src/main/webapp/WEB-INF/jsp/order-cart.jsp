...
<div class="w w1 header clearfix">
    <div id="logo"><a href="/"><img src="/images/logo.png" alt="购淘乐商城"></a></div>
</div>
...
<form id="orderForm" class="hide" action="/order/create.html" method="post">
	...
	<input type="hidden" name="userId" id="userId" value="0"/>
		...
		<input id="hiddenName" type="hidden" name="orderShipping.receiverName" value="张三"/>
		<input id="hiddenPhone" type="hidden" name="orderShipping.receiverMobile" value="15891588888"/>
		<input id="hiddenState" type="hidden" name="orderShipping.receiverState" value="山东省"/>
		<input id="hiddenCity" type="hidden" name="orderShipping.receiverCity" value="青岛"/>
		<%--<input id="hidden" type="hidden" name="orderShipping.receiverDistrict" value="市南区"/>--%>
		<input id="hiddenAddress" type="hidden" name="orderShipping.receiverAddress" value="山东省青岛市市南区中韩街道"/>
</form>
   			...
   			<ul class="ui-switchable-panel-main" id="consignee-list">
				...
				<li class="ui-switchable-panel" id="consignee_index_137617472"
					...
					<div class="consignee-item item-selected"
						...
						<div class="user-name">
							<div class="fl">
								<strong id="strongName" limit="4">张三</strong>&nbsp;&nbsp;收
							</div>
							<div id="divPhone" class="fr">15891588888</div>
							<div class="clr"></div>
						</div>
						<%--<div class="mt10" limit="15">青岛 市南区 中韩街道</div>--%>
						<div id="divAdress" class="adr-m" limit="30">山东省 青岛市 市南区中韩街道</div>
						<div class="op-btns ar" style=" position: absolute;bottom: 5px;right: 5px;">
							...
							<a href="javascript:void(0)" class="easyui-linkbutton" onclick="$('#w').window('open')" style="">编辑</a>
...
<div class="shopping-list ABTest">
	<div class="goods-list">
     <!--配送方式-->
    <h4 class="vendor_name_h" id="0">商家：购淘乐商城</h4>
		...
		<div class="mode-tab-nav">
			<ul>
				<li class="mode-tab-item " id="jd_shipment_item"
					...
					id="jdShip-span-tip" class="m-txt">购淘乐快递<i
						class='qmark-icon qmark-tip'
						data-tips='由购淘乐公司负责配送，速度很快，还接受上门刷卡付款服务'></i></span><b></b></li>
			</ul>
		</div>
	</div>
</div>
    ...
    <div class="ui-ceilinglamp checkout-buttons">
      ...
      <div class="sticky-wrap">
          ...
          <div style="display:none" id="factoryShipCodShowDivBottom" class="dispatching">
            部分商品货到付款方式：先由购淘乐配送“提货单”并收款，然后厂商发货。
          </div>
        </div>
...
<div id="w" class="easyui-window" title="编辑" data-options="modal:true,closed:true,iconCls:'icon-save'" style="width:500px;height:350px;padding:10px;">
		<table cellpadding="5">
			<tr>
				<td>收货人:</td>
				<td><input id="inputName" class="easyui-textbox" type="text" name="receiverName"  style="width: 280px;" value=""/></td>
			</tr>
			<tr>
				<td>收货人手机:</td>
				<td><input id="inputPhone"  class="easyui-numberbox" type="text" name="receiverMobile" data-options="min:0,max:99999999999"/></td>
			</tr>
			<tr>
				<td>省份:</td>
				<td><input id="inputState" class="easyui-textbox" type="text" name="receiverState"  style="width: 280px;"/></td>
			</tr>
			<tr>
				<td>城市:</td>
				<td><input id="inputCity" class="easyui-textbox" type="text" name="receiverCity"  style="width: 280px;"/></td>
			</tr>
			<tr>
				<td>收货地址:</td>
				<td><input id="inputAdress" class="easyui-textbox" type="text" name="receiverAddress"  style="width: 280px;"/></td>
			</tr>
		</table>
			<div style="padding:5px">
				<a href="#none" class="easyui-linkbutton" onclick="submitForm()">提交</a>
			</div>
</div>

<script type="text/javascript">
	// 编辑收货地址，初始值
	$("#inputName").val($("#strongName").text().trim());
	$("#inputPhone").val($("#divPhone").text().trim());

	var adress1=$("#divAdress").text().trim();
	var state;
	var city;
	var adress2;
	var arr=new Array();
	...
	$("#inputCity").val(city);
	$("#inputAdress").val(adress2);

	//编辑收货地址，提交
    function submitForm(){
        //更新收货人
        $("#strongName").text($("#inputName").val().trim());
        $("#hiddenName").val($("#inputName").val().trim());
        //更新手机号
		$("#divPhone").text($("#inputPhone").val().trim());
		$("#hiddenPhone").val($("#inputPhone").val().trim());
        //更新收货地址
        $("#hiddenState").val($("#inputState").val().trim());
        $("#hiddenCity").val($("#inputCity").val().trim());
        $("#hiddenAddress").val($("#inputAdress").val().trim());

        var address3=$("#inputState").val().trim()+" "+$("#inputCity").val().trim()+" "+$("#inputAdress").val().trim();
        $("#divAdress").text(address3);
